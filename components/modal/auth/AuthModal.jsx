      ...
      >
        ...
        <div className={popStyles.popup}>
          <div className={popStyles.auth_header_wrapper}>
            <div onClick={() => setStage('main')}>
              {stage === 'main' ? '' : authIcons.back}
            </div>
            <figure>
              <Image src={logo_icon} alt="" width="100" />
            </figure>
            <div onClick={closeHandler} className={popStyles.cancel_btn}>
              {authIcons.cancel_modal}
            </div>
          </div>
          {stage === 'main' && (
            <>
              <div className={popStyles.login_actions_wrapper}>
                <div>
                  Login to <span className={popStyles.gradient}>LYKLUK</span>
                </div>
                <div
                  onClick={() => setStage('email-login')}
                  className={popStyles.login_act}
                >
          ...
          {stage === 'email-login' && (
            <>
              <EmailPhoneLogin onFinish={() => closeHandler()} />
            </>
          )}

          {stage === 'email-signup' && (
            <>
              <EmailPhoneSignup onFinish={() => closeHandler()} />
            </>
          )}
          ...
          {stage !== 'email-signup' && (
            <div className={popStyles.auth_box_footer}>
              <span>
                ...
                <b onClick={() => setStage('email-signup')}>Sign Up</b>
              </span>
            </div>
          )}
        </div>
